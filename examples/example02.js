...
function main() {
    ...
    var state = {
        ...
        light: {
            position: vec3.fromValues(3.0, 3.0, 3.0),
            intensity: 1.0,
        },
        canvas: canvas,
    };

    setupLightButton("#lightButton", state);
...
function drawScene(gl, deltaTime, state, programInfo, buffers) {
    ...
    // Update uniforms
    {
        ...
        // Lighting uniforms
        gl.uniform3fv(programInfo.uniformLocations.lightPostion, state.light.position);
        gl.uniform1f(programInfo.uniformLocations.lightIntensity, state.light.intensity)
...
function setupLightButton(buttonId, state){
    // Hook up the button
    const button = document.querySelector(buttonId);
    button.addEventListener("click", () => {
        // TODO: Change the light's state here
...
function intializeShaderProgram(gl){
    ...
    const fsSource =
        ...
        void main() {
            // Get the dirstion of the light relative to the object
            ...
            vec3 diffuse = vec3(diff, diff, diff) * uLightIntensity;
    ...
    const programInfo = {
        ...
        uniformLocations: {
            ...
            lightPostion: gl.getUniformLocation(shaderProgram, 'uLightPosition'),
...
function initBuffers(gl, programInfo) {
    ...
    const MAX_LAT = 0.9;
    const LAT_STEP = 0.1;
    const LONG_STEP = 10.0;

    // Make a cylinder
    for(var lat = -MAX_LAT; lat < MAX_LAT; lat += LAT_STEP){
        for(var long = 0; long <= 360.0; long += LONG_STEP){
            var radius = 1.0
            var x = radius * Math.cos(long * Math.PI / 180.0);
            var y = lat;
            var z = Math.sin(long * Math.PI / 180.0);
            positions.push(x, y, z);
            var normal = vec3.fromValues(x, 0.0, z); // Origin is (0,0,0)
            vec3.normalize(normal, normal);
            normals.push(normal[0], normal[1], normal[2]);
        }
    }

    for(var i = 0; i < (MAX_LAT * 2) / LAT_STEP - 1; i++){
        for(var j = 0; j < 360.0 / LONG_STEP; j++) {
            var first = (i * (360.0 / LONG_STEP + 1)) + j;
            var second = first + 360.0 / LONG_STEP + 1;
    ...
    return {
        ...
        attributes: {
            position: initPositionAttribute(gl, programInfo, positionsArray),
            normal: initNormalAttribute(gl, programInfo, normalsArray),
        },
